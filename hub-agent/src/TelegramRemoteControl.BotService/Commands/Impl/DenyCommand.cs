...
public class DenyCommand : ICommand
{
    ...
    public DenyCommand(IOptions<BotSettings> settings)
    {
        _settings = settings.Value;
    }

    public string Id => "deny";
    public string[] Aliases => new[] { "/deny" };
    public string Title => "Запретить";
    public string? Icon => "⛔";
    public string? Description => "Запретить доступ пользователю";
    ...
    public async Task ExecuteAsync(CommandContext ctx)
    {
        ...
        {
            await ctx.ReplyWithBack("❌ Укажите userId: `/deny 123456`");
            return;
        }
        ...
        await ctx.ReplyWithBack($"✅ Доступ пользователя {userId} запрещён");
    }
