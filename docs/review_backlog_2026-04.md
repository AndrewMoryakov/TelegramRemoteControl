...
### BL-05. `ConfirmCallbackHandler` — double-execute деструктивных команд ✅ исправлено
...
### BL-06. Zombie-agent после рестарта Hub — `Heartbeat` молча игнорируется ✅ исправлено
...
### BL-08. Нет retry на первичный `RegisterAgent` failure ✅ исправлено
...
1. Флаг `_isRegistered` в heartbeat-loop — отклонён как смешивание ответственностей (heartbeat-loop должен слать heartbeat, а не ретраить регистрацию).
2. ACK от Hub (`Task<bool> RegisterAgent`) — правильно архитектурно, но breaking change в `IAgentHubServer`.
3. Push от Hub (`ForceReRegister` на клиент) — тоже требует нового метода в контракте.

**Выбран вариант:** объединить Connect + Register в один retry-loop.

**Фикс (реализован):**
1. Внутренний `try/catch` из `AgentService.RegisterAgent` убран — метод теперь пробрасывает исключение.
2. В `ExecuteAsync` объединены `StartAsync` + `RegisterAgent` в один while-retry; при ошибке — `StopAsync` и повтор.
3. В `Reconnected` handler добавлен локальный `try/catch` вокруг `RegisterAgent` (там нет внешнего цикла — если упадём, следующий heartbeat попадёт на unknown connection → BL-06 абортит → сработает `Reconnected` снова).

**Пробел без BL-06:** если `InvokeAsync("RegisterAgent", ...)` вернулся без исключения, а Hub внутри вызвал `Context.Abort()` (невалидный credential, rate-limit), клиент не узнает о провале. С **включённым BL-06** это отлавливается при первом heartbeat — `Abort()` → `Closed/Reconnected` → повторная регистрация.

**Тест:** искусственно вернуть 500 из `RegisterAgent` в хабе (dev-флаг); агент должен ретраить каждые 5 секунд, в логах — `Connect+Register failed. Retrying in 5s...`.
